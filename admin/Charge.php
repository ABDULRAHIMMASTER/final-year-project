...
<?php //include('rgstC.php'); ?>
	...
	<button class="btn btn-primary" onClick="window.print();"><i class="icon-save icon-large"></i>Print Details</button>
                            ...
                            <tbody>
								<?php
								// "select *, sum(paid_amount)as payd from shop join licence on licence.shop_no = shop.shop_no join payment on payment.licence_no = licence.licence_no join receipt on receipt.receipt_no = payment.receipt_no "
								//$result= mysql_query("select * from shop inner join tz_member_shops on shop.shop_no = tz_member_shops.shop_no inner join tz_members on tz_member_shops.tz_members_id = tz_members.id order by shop.id ASC" ) or die (mysql_error());
								$result= mysqli_query($conn, "SELECT *, SUM(paid_amount) AS payd 
					          FROM shop
					          JOIN licence ON licence.shop_no = shop.shop_no
					          JOIN payment ON payment.licence_no = licence.licence_no
					          JOIN receipt ON receipt.receipt_no = payment.receipt_no
					          GROUP BY shop.id") or die (mysqli_error());
